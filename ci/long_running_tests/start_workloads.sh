...
MULTI_CLUSTER_CONFIG="config-multi-node.yaml"
...
# Start the multi-node workloads running.
for workload_file in "$ROOT_DIR"/multi_node_workloads/*; do
  file_name=$(basename -- "$workload_file")
  workload_name="${file_name%.*}"
  (
      # Copy the workload to the cluster.
      ray rsync_up $MULTI_CLUSTER_CONFIG --cluster-name="$workload_name" "$workload_file" "$file_name"
      # Clean up previous runs if relevant.
      ray exec $MULTI_CLUSTER_CONFIG --cluster-name="$workload_name" "source activate tensorflow_p36 && ray stop; rm -r /tmp/ray; tmux kill-server | true"
      # Start the workload.
      ray exec $MULTI_CLUSTER_CONFIG --cluster-name="$workload_name" "source activate tensorflow_p36 && python $file_name" --tmux
   ) &
done
