...
class PopulationBasedTraining(FIFOScheduler):
    ...
    def on_trial_result(self, trial_runner, trial, result):
        ...
        self._checkpoints.append((trial, trial.config, "{} (score {})".format(
            trial, state.last_score), state.last_score, state.last_checkpoint,
                                  time))
        #        else:
        ...
        if self._is_bad(trial, score, time):
            ...
            config, desc, _, checkpoint, t = self._get_good_checkpoint(
                time, trial)
            self._do_exploit(trial_runner.trial_executor, trial, config, desc,
                             checkpoint, t)
...
#                return TrialScheduler.PAUSE  # yield time to other trials
        ...
        self._do_exploit(trial_executor, trial, trial_to_clone.config, desc,
    ...
    def _is_bad(self, trial, score, time):
        ...
        if len(trials) < 2:
            return False
        lower, upper = (trials[:int(math.ceil(len(trials) * PBT_QUANTILE))],
    ...
    def _exploit(self, trial_executor, trial, trial_to_clone):
        ...
        if not new_state.last_checkpoint:
            ...
            return
        desc = "{} (score {})".format(trial_to_clone, new_state.last_score)
        self._do_exploit(trial_executor, trial, trial_to_clone.config, desc,
                         new_state.last_checkpoint,
                         new_state.last_perturbation_time)

    def _do_exploit(self, trial_executor, trial, config, checkpoint_desc,
                    checkpoint, t):
        trial_state = self._trial_state[trial]
        ...
        logger.info("[exploit] transferring weights from trial "
                    "{} -> {} (score {})".format(checkpoint_desc, trial,
        ...
        reset_successful = trial_executor.reset_trial(trial, new_config,
                                                      new_tag)
        if reset_successful:
            trial_executor.restore(trial, Checkpoint.from_object(checkpoint))
        else:
            trial_executor.stop_trial(trial, stop_logger=False)
            trial.config = new_config
            trial.experiment_tag = new_tag
            trial_executor.start_trial(trial,
                                       Checkpoint.from_object(checkpoint))
