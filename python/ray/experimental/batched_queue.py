    ...
    def put_next(self, item):
        with self.flush_lock:
            if self.background_flush and not self.flush_thread.is_alive():
                logger.info("[writer] Starting batch flush thread")
...
@ray.remote
class Reader(object):
    def __init__(self, queue, max_reads_per_second=999999):
        self.queue = queue
...
def test_max_throughput():
    ...
    reader = Reader.remote(queue, max_reads_per_second=float("inf"))
...
def test_backpressure():
    queue = BatchedQueue(
        ...
        prefetch_depth=10,
        background_flush=False)
    reader = Reader.remote(queue, max_reads_per_second=50000)
    reader.read_forever.remote()
    value = 0
    for _ in range(5):
        N = 100000
