...
def add_car_snow(obs, num_snow=30, noise=False):
    global num_rendered
    num_rendered += 1
    ...
    if type(obs) in [str, unicode]:
        obs = unpack(obs)
    if noise:
        noise_vec = np.random.randint(50, size=(80, 80, 1), dtype=np.uint8)
        obs = obs
        obs = obs + noise_vec
    if num_snow:
#        obs = 255 - obs
        obs = create_snow_dynamic(num_snow, obs, 9, snow_size=8, intensities=[50, 100])
